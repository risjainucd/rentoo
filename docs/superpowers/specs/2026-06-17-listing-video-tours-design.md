...
// Resolved by metadata probe (longest clip <= ~120s) + eyeball at --sample.
// Only #01 and #37 need an override; #03/#44/#86/#88 natural-sort-first already lands the intended clip
// (#86's 38s '4.53.50' clip sorts before the 333s '4.53.53' monster, so no override needed).
const CLIP_OVERRIDE = {
  '#01': 'WhatsApp Video 2026-04-07 at 12.45.06 PM.mp4', // 59s walkthrough; 'af.mp4' (34s) sorts first but is partial
  '#37': 'WhatsApp Video 2026-04-08 at 8.07.20 PM.mp4',  // 62s; '8.07.02' (13s) sorts first/too short, '8.07.40' is 361s/68MB
};
...
1. **Multi-video folders** — pick the longest clip ≤ ~120s (`CLIP_OVERRIDE` pre-filled for `#01`→59s and `#37`→62s; `#03/#44/#86/#88` use natural-sort-first), then confirm/adjust visually at the `--sample` step before the full upload run.
2. **"56 vs 49"** — confirmed: one tour per listing (49 tours). The 7 surplus clips in multi-video folders are dropped, not concatenated.
3. **Audio** — **keep** at AAC 96k; plays on tap (muted until the user presses play). No mute/normalize step.

**Open question (deferred, not blocking v1):**
- **Lightbox/full view:** the spec uploads `tour-full.webp` for parity but the current Gallery has no full-screen lightbox. Inline-in-stage playback is sufficient for v1; a fullscreen video view can be a later enhancement.
